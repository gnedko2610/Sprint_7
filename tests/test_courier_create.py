...
from data.test_data import TEST_COURIER_DATA
import allure
...
class TestCourierCreate:
    ...
    def test_create_courier_success(self, courier_cleanup):
        ...
        response = CourierAPI.create_courier_with_cleanup(login, password, first_name, courier_cleanup)
    ...
    def test_create_courier_duplicate(self, courier_cleanup):
        courier_data = CourierAPI.register_new_courier_and_return_login_password()
        login, password, first_name = courier_data
        courier_cleanup.append((login, password))
    ...
    def test_create_courier_missing_firstname(self, courier_cleanup):    
        login = generate_random_string(10)
        password = generate_random_string(10)
        response = CourierAPI.create_courier_with_cleanup(login, password, None, courier_cleanup)
        assert response.status_code == ResponseCode.CREATED
        assert response.json() == {"ok": True}
    ...
    def test_create_courier_existing_login(self, courier_cleanup):       
        courier_data = CourierAPI.register_new_courier_and_return_login_password()
        login, password, first_name = courier_data
        courier_cleanup.append((login, password))
        new_password = TEST_COURIER_DATA["new_password"]
        new_first_name = TEST_COURIER_DATA["new_first_name"]
